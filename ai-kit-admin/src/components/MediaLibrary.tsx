...
function ensureModalBox() {
  ...
  let host = details.querySelector("#smartcloud-ai-kit-media-modal-box");
  if (!host) {
    host = document.createElement("div");
    host.id = "smartcloud-ai-kit-media-modal-box";
...
function ensureGridBulkButton() {
  ...
  if (!isGridBulkSelectMode()) {
    const old = document.getElementById("smartcloud-ai-kit-grid-preview-btn");
    if (old) old.remove();
    return;
  }
  ...
  let btn = document.getElementById(
    "smartcloud-ai-kit-grid-preview-btn",
  ) as HTMLButtonElement | null;
  if (!btn) {
    btn = document.createElement("button");
    btn.id = "smartcloud-ai-kit-grid-preview-btn";
    btn.type = "button";
    ...
    btn.addEventListener("click", () => {
      ...
      const hostId = "smartcloud-ai-kit-bulk-overlay-host";
...
function initAttachmentMetabox() {
  const host = document.getElementById(
    "smartcloud-ai-kit-attachment-metabox-root",
  );
  if (!host) return;
